...
Then
\[\oint_{\mathcal{C}} \frac{f'(z)}{f(z)} \mathrm{d}z = 2\pi i \sum_{z\in\mathop{interior}(\mathcal{C})} \ord_z(f)\ (=2\pi i \cdot(\textrm{number of zeroes} - \textrm{number of poles, with multiplicity})).\]
\end{thm}
