...
\usepackage{amsfonts, amsmath, amssymb, amsthm, hyperref, enumerate}
\usepackage[all]{xy}
...
So $0 \to \ZZ \to 0$ is the chain complex $C_\bullet^\Delta(X)$.
So
\[
...
\[
H_\bullet^\Delta(X) \cong H_\bullet^\text{sing}(X).
\]
...
Define the \emph{reduced homology groups} $\tilde H_\bullet (X)$ to be the homology groups of this chain complex.
...
Recall that $H_\bullet (X\sqcup Y) = H_\bullet(X) \oplus H_\bullet(Y)$, the reduced homology groups behave nicely with respect to many operations such as 1-point unions.
In a 1-point union $X\vee Y = X\sqcup Y /x\sim y$ for some designated point $x\in X$ and $y\in Y$.
So $\tilde H_\bullet(X\vee Y) = \tilde H_\bullet(X) \oplus \tilde H_\bullet(Y)$.
...
\begin{exer}
If $X \xrightarrow{f} Y \xrightarrow{g} Z$ then $(g\circ f)_n = g$ %TODO
\end{exer}
